...
- As chaves de resposta usam `make_plan_cache_key(build_id, scope, entity, plan_hash)`, onde o `plan_hash` resume entity, intent, bucket, scope, identifiers, agg_params, flags e métricas pedidas; não entra `conversation_id` e só entra `client_id` se estiver copiado para identifiers.【F:app/cache/rt_cache.py†L268-L315】【F:app/cache/rt_cache.py†L376-L387】
...
- **Como provar:** Popular manualmente uma chave com `make_plan_cache_key` (vide seção 4) e repetir a pergunta com o runner; `meta.cache.hit` deve vir `true`, `cache_get_outcome=hit` no log e não aparece latência de executor (`sirios_sql_query_duration_seconds`).
...
- **Padrão observado vs código:** As chaves vistas no Redis seguem `araquem:dev-20251030:cfg-<hash>:<pub|prv>:<entity>:plan:<hash>`. O código gera `plan_hash` com entity, intent, bucket, scope, identifiers normalizados, `agg_params`, flags e métricas pedidas e monta a chave com `make_plan_cache_key(build_id, scope, entity, plan_hash)`, o que explica o segmento `cfg-<hash>:<scope>:<entity>:plan:<hash>` observado.【F:app/cache/rt_cache.py†L268-L315】【F:app/cache/rt_cache.py†L376-L387】
