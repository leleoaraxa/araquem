...
- **Grafana** — visualização com dashboards provisionados via scripts.【F:docker-compose.yml†L105-L125】【F:scripts/observability/gen_dashboards.py†L1-L104】
...
## 3. Scripts & Crons
### Ciclo de qualidade
- `scripts/quality/quality_push_cron.py` — Varre `data/ops/quality/*.json`, valida esquema (routing/projection/RAG), envia para `/ops/quality/push`. Aceita `--dry-run` e respeita `QUALITY_SAMPLES_GLOB`. Rodar manualmente para testar novos datasets.【F:scripts/quality/quality_push_cron.py†L1-L181】
- `docker/quality-cron.sh` — Entry point do container homônimo; instala PyYAML, espera API saudável, roda `quality_push_cron.py`, aguarda métricas Prometheus (`top1_accuracy`, `routed_rate`) e executa `scripts/quality/quality_gate_check.sh` a cada hora.【F:docker/quality-cron.sh†L1-L27】
- `scripts/quality/quality_gate_check.sh` — Chama `validate_data_contracts.py` e consulta `/ops/quality/report`, falhando se o status não for `pass`. Útil para rodar localmente antes de merges.【F:scripts/quality/quality_gate_check.sh†L1-L16】
- `scripts/core/validate_data_contracts.py` — Garante que `data/golden/m65_quality.yaml` e `.json` estão em sincronia. Rodar após edições do golden set.【F:scripts/core/validate_data_contracts.py†L1-L67】

### RAG e embeddings
- `scripts/embeddings/embeddings_build.py` — Lê `data/embeddings/index.yaml`, chunkiza arquivos, chama Ollama para gerar embeddings e escreve `store/embeddings.jsonl` + `manifest.json`. Rodar após atualizar ontologia, entidades ou conceitos usados pelo RAG.【F:scripts/embeddings/embeddings_build.py†L1-L93】【F:data/embeddings/index.yaml†L1-L52】
...
- `docker/rag-eval-cron.sh` — Script utilitário para rodar `scripts/embeddings/rag_retrieval_eval.py` dentro do cluster e registrar métricas agregadas (Recall@K, MRR, NDCG). Pode ser disparado ad-hoc pós-refresh.【F:docker/rag-eval-cron.sh†L1-L8】【F:scripts/embeddings/rag_retrieval_eval.py†L1-L87】
- `scripts/embeddings/rag_retrieval_eval.py` — Calcula métricas em `data/ops/quality_experimental/rag_eval_set.json`, consulta índice, registra resultado via `/ops/metrics/rag/eval/register` e persiste última execução em `data/ops/quality_experimental/rag_eval_last.json`. Use `--eval`, `--k` e `--index` para personalizar.【F:scripts/embeddings/rag_retrieval_eval.py†L1-L87】

### Observabilidade
- `scripts/observability/gen_dashboards.py` — Renderiza dashboards Grafana usando templates Jinja e `data/ops/observability.yaml`. Gere novamente após editar bindings, thresholds ou templates.【F:scripts/observability/gen_dashboards.py†L1-L104】【F:data/ops/observability.yaml†L1-L90】
- `scripts/observability/gen_alerts.py` — Constrói `prometheus/recording_rules.yml` e `alerting_rules.yml` a partir da mesma fonte YAML. Execute antes de subir Prometheus ou após mudar thresholds.【F:scripts/observability/gen_alerts.py†L1-L78】【F:prometheus/recording_rules.yml†L1-L28】
- `scripts/observability/obs_audit.py` — Auditoria que valida a existência e frescor de dashboards/regras, ausência de placeholders e cobertura de bindings/thresholds. Ideal para CI e para checar drift de observabilidade.【F:scripts/observability/obs_audit.py†L1-L93】
- `scripts/quality/gen_quality_dashboard.py` — Gera painel específico de quality gates com thresholds de `planner_thresholds.yaml`. Útil para inspeção rápida dos indicadores críticos.【F:scripts/quality/gen_quality_dashboard.py†L1-L98】

### Golden & governança
- `scripts/core/golden_sync.py` — Normaliza YAML de amostras (`--in`) para JSON estável (`--out`), garantindo ordenação e campos obrigatórios. Use ao atualizar datasets routing/golden.【F:scripts/core/golden_sync.py†L1-L87】
- `scripts/observability/hash_guard.py` — Calcula hashes de `data/ontology` e `data/golden`, mantendo `.hash` atualizado para detectar drift. Rodar após mudanças planejadas nos contratos.【F:scripts/observability/hash_guard.py†L1-L33】

### Utilitários adicionais
- `scripts/quality/gen_quality_dashboard.py` (ver acima) e `scripts/maintenance/warmup.ps1` (aquecer cache em ambientes Windows).
- `scripts/quality/quality_push.py` — CLI simples para enviar payloads JSON/YAML específicos para `/ops/quality/push` com cabeçalho de token. Útil para reprocessar amostras isoladas.【F:scripts/quality/quality_push.py†L1-L47】
- `scripts/quality/quality_push_cron.py` também é usado fora do cron para inspeção (`--dry-run`).
...
- `scripts/` — Automatizações reproduzíveis para qualidade, observabilidade e RAG (detalhadas acima).【F:scripts/observability/gen_dashboards.py†L1-L104】
...
- **Rodar quality manualmente:** `docker compose run --rm quality-cron /usr/local/bin/quality-cron.sh` ou local `python scripts/quality/quality_push_cron.py && scripts/quality/quality_gate_check.sh`. Use `QUALITY_SAMPLES_GLOB` para escopar datasets.【F:docker/quality-cron.sh†L1-L27】【F:scripts/quality/quality_push_cron.py†L1-L181】
- **Reindexar embeddings:** `docker compose run --rm rag-indexer` (usa CMD do Dockerfile) ou `python scripts/embeddings/embeddings_build.py --index data/embeddings/index.yaml --out data/embeddings/store`. Confirmar `manifest.json` atualizado e métricas RAG via `/ops/metrics/rag/register`.【F:docker/Dockerfile.rag-indexer†L1-L20】【F:scripts/embeddings/embeddings_build.py†L1-L93】
- **Regenerar dashboards/alerts:** `python scripts/observability/gen_dashboards.py --config data/ops/observability.yaml --out grafana/dashboards` e `python scripts/observability/gen_alerts.py --config data/ops/observability.yaml`. Rodar `scripts/observability/obs_audit.py` para validar saída.【F:scripts/observability/gen_dashboards.py†L47-L98】【F:scripts/observability/gen_alerts.py†L50-L75】【F:scripts/observability/obs_audit.py†L37-L76】
- **Validar dados D-1:** Executar `python scripts/core/validate_data_contracts.py` e (se banco disponível) rodar queries canônicas via `/ops/cache/bust` + `/ask` para verificar latências e dados recentes conforme contratos em `data/entities`.【F:scripts/core/validate_data_contracts.py†L1-L67】【F:docs/dev/GUARDRAILS_ARAQUEM.md†L57-L75】
...
| `rag metrics 0.0` | Índice desatualizado ou refresh falhou | `curl -s -X POST localhost:8000/ops/metrics/rag/register -d '{"store":"data/embeddings/store/embeddings.jsonl"}'` | Rodar `rag-indexer`, garantir Ollama online, revisar logs do cron.【F:docker/rag-refresh-cron.sh†L1-L20】【F:scripts/embeddings/embeddings_build.py†L1-L93】 |
...
| `obs_audit` falha | Dashboards/alerts desatualizados vs `observability.yaml` | `python scripts/observability/obs_audit.py` | Regenerar com `gen_dashboards.py`/`gen_alerts.py`.【F:scripts/observability/obs_audit.py†L37-L84】 |

## 9. Automatizações seguras (futuras)
- **CI para observabilidade:** adicionar job pós-merge que roda `scripts/observability/gen_dashboards.py` e `scripts/observability/gen_alerts.py` seguido de `scripts/observability/obs_audit.py`, garantindo que os artefatos estejam sempre alinhados com `observability.yaml` sem tocar em código.【F:scripts/observability/gen_dashboards.py†L47-L98】【F:scripts/observability/gen_alerts.py†L50-L75】【F:scripts/observability/obs_audit.py†L37-L84】
...
- **RAG eval scheduled:** usar `docker/rag-eval-cron.sh` como base para pipeline que registra métricas em Prometheus após cada refresh, alimentando painéis automáticos.【F:docker/rag-eval-cron.sh†L1-L8】【F:scripts/embeddings/rag_retrieval_eval.py†L1-L87】
