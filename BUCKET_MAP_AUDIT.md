...
1) **Bucket A/B/C/D (governance)** será fonte única em `data/ontology/entity.yaml`, onde há blocos `buckets` (entidades) e `intents[].bucket` (intents). (data/ontology/entity.yaml:L13-L44) (data/ontology/entity.yaml:L41-L120)
2) Hoje existe **duplicidade** com `data/ontology/bucket_rules.yaml`, que também define buckets A/B/C/D e está **desativado** (`enabled: false`). (data/ontology/bucket_rules.yaml:L19-L87)
...
7) A policy de RAG assume Bucket D para domínios textuais e o Planner consulta bucket em `rerank.domains`. (data/policies/rag.yaml:L51-L63) (app/planner/planner.py:L221-L266)
...
9) A convergência exige **reservar “bucket” apenas para D1** (A/B/C/D) e **renomear** demais usos (D2/D3/D4) no plano (sem código). (data/ops/observability.yaml:L74-L179) (data/entities/catalog.yaml:L61-L66)
10) Ações de convergência (sem código): (i) congelar A/B/C/D em `entity.yaml`, (ii) declarar “bucket” reservado, (iii) renomear usos D2/D3/D4, (iv) orientar consumidores a ler somente `entity.yaml`, (v) descontinuar `bucket_rules.yaml` no futuro. (data/ontology/entity.yaml:L13-L44) (data/ontology/bucket_rules.yaml:L19-L87)
...
- Duplicidade: bucket_rules.yaml com A/B/C/D desativado. (data/ontology/bucket_rules.yaml:L19-L87)
...
**D1) bucket_governance_abcd (A/B/C/D)**
- Definição oficial proposta: bucket A/B/C/D definido em `data/ontology/entity.yaml` (buckets e intents). (data/ontology/entity.yaml:L13-L44)
...
**Evidências‑chave**
- Fonte A/B/C/D: `buckets` + `intents[].bucket`. (data/ontology/entity.yaml:L13-L44)
...
**Provas**
- Bloco `buckets` (A/B/C/D) com entidades agrupadas. (data/ontology/entity.yaml:L13-L39)
...
- **Bucket D**: vazio no YAML. (data/ontology/entity.yaml:L39-L39)

**Evidências‑chave**
- `buckets` A/B/C/D em entity.yaml. (data/ontology/entity.yaml:L13-L39)
...
- **RAG/rerank** consulta bucket via `domains` na policy. (app/planner/planner.py:L221-L266)
...
|---|---|---|---|---|---|
| app/planner/planner.py | resolve_bucket | Resolve bucket A/B/C/D | D1 | Escreve | (app/planner/planner.py:L40-L103) |
...
- **Tests D1**: ContextManager e Narrator dependem de bucket A/B/C/D. (tests/context/test_context_last_reference_multi_ticker.py:L12-L49) (tests/narrator/test_narrator_bucket_d_llm.py:L14-L70)
- **Tests D5**: Quota usa “bucket” como estrutura de storage local. (tests/quota/test_ask_quota_enforcement.py:L34-L43)
...
- Scripts/Tests com bucket (D1/D5). (scripts/quality/build_coverage_report.py:L75-L92) (tests/quota/test_ask_quota_enforcement.py:L34-L43)
...
3) `data/policies/narrator.yaml` → D1 (buckets A/B/C/D). (data/policies/narrator.yaml:L38-L55)
4) `data/policies/rag.yaml` → D1 (Bucket D em notes). (data/policies/rag.yaml:L51-L63)
...
- **Evidência de existência**: bucket_rules define A/B/C/D e está desativado. (data/ontology/bucket_rules.yaml:L19-L87)
...
**Duplicação #2 — quaisquer outras “fontes” de bucket A/B/C/D**
- **Docs** que afirmam bucket A/B/C/D (não-fonte):
...
- Docs com bucket A/B/C/D. (docs/dev/entities/fiis_rankings.md:L160-L163)
...
1) **Congelar definição A/B/C/D em `entity.yaml`** (buckets + intents). (data/ontology/entity.yaml:L13-L44)
   - Prós: fonte única explícita. Contras: exige disciplina para não replicar em docs/policies.
2) **Declarar “bucket” como termo reservado para D1** (A/B/C/D). (data/ontology/entity.yaml:L13-L44)
...
4) **Consumidores D1 devem ler exclusivamente `entity.yaml`** (Planner/Orchestrator/Context/Narrator/RAG). (app/planner/planner.py:L453-L503) (app/orchestrator/routing.py:L357-L470)
...
1) **Nenhum arquivo fora de `data/ontology/entity.yaml` define bucket A/B/C/D** (apenas referências derivadas). (data/ontology/entity.yaml:L13-L44)
...
4) **`plan_hash` inclui bucket A/B/C/D apenas quando proveniente de `entity.yaml`**. (app/cache/rt_cache.py:L323-L370)
5) **ContextManager aplica TTL por bucket A/B/C/D sem depender de fontes duplicadas**. (app/context/context_manager.py:L586-L607) (data/policies/context.yaml:L63-L83)
6) **Narrator e RAG leem bucket apenas como D1** (policy por bucket e rerank domains). (app/narrator/narrator.py:L860-L938) (app/planner/planner.py:L221-L266)
...
- [ ] `entity.yaml` é a única fonte de bucket A/B/C/D. (data/ontology/entity.yaml:L13-L44)
...
- `entity.yaml` contém buckets A/B/C/D e intents[].bucket. (data/ontology/entity.yaml:L13-L44)
