...
A ontologia exclui termos relacionados a **preço, dividendos, risco, ranking, vacância, DY, notícias, etc.**, para evitar colisão com outras intents como `fiis_precos`, `fiis_dividends`, `fiis_financials_snapshot`, `fiis_rankings`, `fiis_real_estate`, `fiis_noticias`.
