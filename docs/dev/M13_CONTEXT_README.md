...
- As intents `fiis_financials_risk`, `fiis_overview` e `fiis_quota_prices` agora possuem regras declarativas em `data/ops/param_inference.yaml` que priorizam extração de ticker pelo texto e recorrem ao `last_reference` do contexto como fallback quando configuradas para isso.【F:data/ops/param_inference.yaml†L90-L137】【F:data/ops/param_inference.yaml†L290-L309】
