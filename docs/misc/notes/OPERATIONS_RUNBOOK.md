...
- `scripts/embeddings/rag_retrieval_eval.py` — Calcula métricas em `data/ops/quality/rag_eval_set.json`, consulta índice, registra resultado via `/ops/metrics/rag/eval/register` e persiste última execução em `data/ops/quality/rag_eval_last.json`. Use `--eval`, `--k` e `--index` para personalizar.【F:scripts/embeddings/rag_retrieval_eval.py†L1-L87】
