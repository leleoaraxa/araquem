...
class SimpleEnvironment:  # pragma: no cover - exercised in fallback mode
    def __init__(self, template_dir: str, labels: Dict[str, str]):
        ...
        self.global_context = {"promql_filter": lambda keys: promql_filter(keys, labels)}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Grafana dashboards from YAML bindings")
    parser.add_argument("--config", required=True, help="Path to observability YAML configuration")
    parser.add_argument("--out", required=True, help="Output directory for rendered dashboards")
    parser.add_argument(
        "--templates",
...
def ensure_required_keys(config: Dict) -> None:
    missing_sections = [section for section in ("bindings", "labels", "thresholds") if section not in config]
    if missing_sections:
...
def render_dashboards(env: Environment, config: Dict, output_dir: Path) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    render_context = config if HAS_JINJA else {k: to_namespace(v) for k, v in config.items()}
