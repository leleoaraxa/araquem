        ...
        "options": {
            ...
            "thresholds": {
                ...
                "steps": [
                    ...
                    *[{"value": v, "color": c} for (v, c, *_) in thresholds[1:]],
                ],
            },
        },
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate Grafana dashboard from YAML thresholds"
    )
    parser.add_argument(
        "--pretty", action="store_true", help="Emit pretty-printed JSON (indent=2)"
    )
    ...
    q_conf_heat = (
        "sum by (expected_intent, predicted_intent) (sirios_planner_confusion_total)"
    )
    ...
    # Misses (stat)
    db["panels"].append(singlestat_panel("Misses (abs)", q_miss_abs, "none", x=0, y=6))
    db["panels"].append(
    ...
    db["panels"].append(
        heatmap_panel(
            "Confusion Matrix — expected × predicted", q_conf_heat, x=0, y=10, h=10
        )
    )

    # Metadata (read-only, título com versão/janela)
    version = str(policy.get("version", "unknown"))
    window = str(policy.get("window", "rolling"))
